...
import { AlertTriangle, Edit3, Download, Upload, LogOut } from "lucide-react";
import type { AppSettings } from "../types";
import { useAuth } from "@/contexts/AuthContext";
import { useRouter } from "next/navigation";
import ProfileAvatarButton from "./settings/ProfileAvatarButton";
...
export default function SettingsPage({
  ...
  return (
    <div className="space-y-4">
      {/* Header Row: Analytics, Title, Profile */}
      <div className="flex items-center justify-between w-full mb-4">
        ...
        <div className="flex-1 flex justify-center items-center">
          <span className="matrix-font-large text-green-400">TASKJAR</span>
        </div>
        {/* Profile Avatar Button - Right */}
        <div className="flex justify-end">
          <ProfileAvatarButton playSound={playSound} />
        </div>
      </div>
      {/* Analytics Modal */}
      <Dialog open={showAnalytics} onOpenChange={setShowAnalytics}>
        <DialogContent className="max-w-2xl p-0 bg-black/90 glass spotify-glow border border-green-500/30 shadow-2xl rounded-3xl overflow-hidden">
          <div className="flex flex-col items-center justify-center w-full h-full p-8">
            <h2 className="matrix-font-large text-green-400 mb-6 drop-shadow-lg">
              Your Weekly Progress
            </h2>
            <ChartContainer
              config={{ value: { color: "#1DB954", label: "Tasks Completed" } }}
              className="w-full h-80 bg-black/80 rounded-2xl shadow-xl p-6 spotify-glow"
            >
              {({
                ResponsiveContainer,
                BarChart,
                ...
                <BarChart data={analyticsData}>
                  <XAxis
                    ...
                    tick={{
                      fill: "#1DB954",
                      fontWeight: 600,
                      fontFamily: "MinecartLCD, monospace",
                    }}
                  />
                  <YAxis
                    axisLine={false}
                  ...
                  />
                  <Tooltip
                    cursor={{ fill: "#1DB954", opacity: 0.1 }}
                    contentStyle={{
                      background: "#111",
                      borderRadius: 8,
                      color: "#1DB954",
            ...
            <p className="mt-6 text-lg text-green-300 text-center font-mono">
              Keep up the great work! This chart shows your task completion for
              the week in a fun, easy-to-read way.
            </p>
          </div>
        </DialogContent>
      </Dialog>
