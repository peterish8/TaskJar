...
import styles from "./local-landing.module.css"
...
export default function LocalLanding({ onEnter }: { onEnter: () => void }) {
  return <main className="landing"><div>
    ...
    <div className={styles.proof} aria-label="TaskJar privacy and pricing highlights">
      <span><LockKeyhole/> No login or auth</span>
      <span><DatabaseZap/> No Supabase or database</span>
      <span><WalletCards/> Free to use</span>
    </div>
    <p className={styles.note}>Optional speech and planning models download from public model hosts once, then run locally. Personal task data and raw microphone audio are never uploaded by TaskJar.</p>
